              ...
              const infoWindow = new window.google.maps.InfoWindow({
                content: `
                                  <div style="padding: 10px;">
                                      <img src="${
                                        place.placeImage
                                      }" alt="장소 이미지" style="width: 100%; height: 100px; object-fit: cover;">
...
const TravelInfo = () => {
  ...
  const getPlaceList = useCallback(async () => {
    try {
      ...
      setError(null);
      const response = await axiosInstance.get(
        `/api/v1/travels/travelInfos/${travelInfoId}/places`
      );
  ...
  const getUrlPlaceList = useCallback(async (urlId) => {
    try {
      ...
      setError(null);
      const response = await axiosInstance.get(
        `/api/v1/travels/travelInfos/urls/${urlId}`
      );
      setPlaceList(response.data);
    } catch (error) {
      console.error("API Error:", error);
  ...
  const putTravelInfoUpdate = useCallback(
    async (days, title) => {
      try {
        // 값 유효성 검사
        if (!title || !days) {
        ...
        await axiosInstance.put(`/api/v1/travels/travelInfos/${travelInfoId}`, {
    ...
    [travelInfoId, travelInfoTitle, travelDays]
  ); // 의존성 배열에 필요한 값들 추가

  const getAISelect = useCallback(async () => {
    try {
      if (
        selectedAIPlaces.length === 0 ||
        selectedAIPlaces.length !== selectedPlaces.length
      ) {
        setLoading(true);
        setError(null);
        const response = await axiosInstance.get(
          `/api/v1/travels/travelInfos/${travelInfoId}/aiSelect`
        );
  ...
  const checkPlaceType = (itemType) => {
    ...
    if (PLACE_TYPE_CATEGORIES.restaurant.includes(itemType))
      return "restaurant";
    return "etc";
  };
      ...
      <main className="HG-TravelInfo-Container">
        <div className="HG-TravelInfo-Header">
          <div className="WS-TravelInfo-Header-Left">
            <div className="WS-TravelInfo-Header-Left-Back-Btn">
              <img
                className="WS-TravelInfo-Header-Left-Back-Btn-Icon"
                src={backArrowIcon}
                alt="backArrowIcon"
                onClick={() => navigate(-1)}
              />
            ...
            <div className="WS-TravelInfo-Header-Left-Contents">
              ...
              <div className="HG-TravelInfo-Title-Edit-Container">
                ...
                <span
                  className="HG-TravelInfo-Title-Edit-text"
          ...
          <div className="WS-TravelInfo-Header-Right" onClick={handleSelectBtn}>
            ...
            <img src={planeIcon} alt="selectIcon" />{" "}
            {/* FEAT: 선택 버튼 선택 여행지 모달 팝업 */}
            <span className="HG-TravelInfo-Select-Cnt">
        ...
        <div className="HG-TravelInfo-Body">
          <div className="HG-travelinfo-Title-list">
            <span
              className={`HG-travelinfo-content-frame-url ${
                activeSpan === 1 ? "HG-underline" : ""
              }`}
            ...
            {travelInfo.urlList.map((item, index) =>
              item.urlAddress.includes("youtube") ? (
                <span
                  className={`HG-travelinfo-content-frame-url ${
                    activeSpan === `${index + 2}` ? "HG-underline" : ""
                  }`}
                  key={index}
          ...
          <div
            className={` ${
              activeSpan === 1
                ? "HG-TravelInfo-Content-Blank"
                : "HG-TravelInfo-Content-Title"
            }`}
          >
            {(() => {
              try {
          ...
          <div className="HG-TravelInfo-Type-List">
            <span
              className={`${
                placeType === "landmark"
                  ? "HG-TravelInfo-Selected-Type"
                  : "HG-TravelInfo-Unselected-Type"
              }`}
            ...
            <span
              className={`${
                placeType !== "landmark" && placeType !== "restaurant"
                  ? "HG-TravelInfo-Selected-Type"
                  : "HG-TravelInfo-Unselected-Type"
              }`}
          ...
          <div className="HG-TravelInfo-aiselect-btn">
            <span
              className={`${
                isAISelected
                  ? "HG-TravelInfo-aiselect-btn-ai-icon-selected"
                  : "HG-TravelInfo-aiselect-btn-text"
              }`}
              onClick={handleAISelected}
            >
              <img
                className={`HG-TravelInfo-aiselect-btn-ai-icon`}
                src={aiIcon}
                alt="aiIcon"
              />
              AI 추천선택
          ...
          <div className="HG-TravelInfo-Content-Frame-Place-Slider">
            {placeList.content.map((item, index) => {
              ...
              return isMatchingType || isEtcType ? (
                <div
                  key={index}
                  className={`WS-carousel-item ${
                    selectedPlaces.some(
                      (selected) => selected.placeId === item.placeId
                    )
                      ? "HG-select-place"
                      : ""
                  }`}
                >
                  <div
                    className="HG-trevelinfo-select-Container"
                  ...
                  <Slider {...sliderSettings}>
                    ...
                    <div className="slide-content">
                      <img
                        ...
                        onError={(e) => {
                          ...
                          e.target.src =
                            "https://images.unsplash.com/photo-1542051841857-5f90071e7989?ixlib=rb-4.0.3";
                        }}
                    ...
                    <div className="slide-content" key={`map-${index}`}>
                      {item?.latitude && item?.longitude && (
                        <MapComponent key={`map-${index}`} places={[item]} />
                      )}
                    </div>
                  </Slider>
                </div>
        ...
        <div
          className={`${
            isSelectModalOpen ? "HG-TravelInfo-Select-Modal" : "none"
          }`}
        >
          <SelectModal
